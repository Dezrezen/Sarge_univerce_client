...
public class CameraController:MonoBehaviour
{
	...
	[SerializeField]
	[Tooltip("Min ZOOM IN")]
	private float zoomMax = 1.0f;						//caps for zoom
	
	[Tooltip("Max ZOOM OUT")]
	[SerializeField]
...
//	void UpdateFunctionality()
	...
	private void MoveCamera()
	{
		...
		if((Input.GetMouseButton (0) && Input.mousePosition.x < Screen.width && Input.mousePosition.y < Screen.height) && Input.touchCount == 1)
#else 			
	...
	private void MouseZoom()
	{
		...
		float fov = cameratk2D.ZoomFactor;
		fov += Input.GetAxis("Mouse ScrollWheel") * sensitivity;
		fov = Mathf.Clamp(fov, zoomMin, zoomMax);
		cameratk2D.ZoomFactor = fov;
	}
	...
	private void TouchMoveZoom()
	{
		...
		if (Input.touchCount == 2 && Input.GetTouch(0).phase == TouchPhase.Moved//check for 2 fingers on screen and that they are moving
		                          && Input.GetTouch(1).phase == TouchPhase.Moved)
		{
			...
			cameratk2D.ZoomFactor =
				Mathf.Clamp(cameratk2D.ZoomFactor + (difference * 0.001f), zoomMin, zoomMax);
			
			
			// Debug.LogError("size after: " + cameratk2D.ScreenCamera.orthographicSize);
		}
