...
namespace UIControllersAndData.Store
{
    public class ShopData
    {
        ...
        private ArmyCategoryData _armyCategoryData;
        ...
        public ArmyCategoryData ArmyCategoryData
        {
            get
            {
                if (_armyCategoryData == null)
                    _armyCategoryData =
                        (ArmyCategoryData)LoadShopCategoryTable<ArmyCategoryData>(ShopCategoryType.Army);
                return _armyCategoryData;
            }
        }
        ...
        private static object LoadShopCategoryTable<T>(ShopCategoryType categoryType)
        {
            switch (categoryType)
            {
                ...
                case ShopCategoryType.Buildings:
                    return (T)(object)Resources.Load<BuildingsCategoryData>(Constants
                        .PATH_FOR_BUILDINGS_CATEGORY_ASSET_LOAD);
                case ShopCategoryType.Army:
                    return (T)(object)Resources.Load<ArmyCategoryData>(Constants
                        .PATH_FOR_MILITARY_CATEGORY_ASSET_LOAD);
                ...
                case ShopCategoryType.Ambient:
                    return (T)(object)Resources.Load<AmbientCategoryData>(
                        Constants.PATH_FOR_AMBIENT_CATEGORY_ASSET_LOAD);
            }
        ...
        public List<BaseStoreItemData> GetLevels(int id, ShopCategoryType category)
        {
            ...
            {
                case ShopCategoryType.Buildings:
                    return BuildingsCategoryData.category.Find(x => x.id == id).levels
                        .Select(item => item as BaseStoreItemData).ToList();
                case ShopCategoryType.Army:
                    return ArmyCategoryData.category.Find(x => x.id == id).levels
                        .Select(item => item as BaseStoreItemData).ToList();
                case ShopCategoryType.Weapon:
                    return WeaponCategoryData.category.Find(x => x.id == id).levels
                        .Select(item => item as BaseStoreItemData).ToList();
                case ShopCategoryType.Walls:
                    return WallsCategoryData.category.Find(x => x.id == id).levels
                        .Select(item => item as BaseStoreItemData).ToList();
                case ShopCategoryType.Ambient:
                    return AmbientCategoryData.category.Find(x => x.id == id).levels
                        .Select(item => item as BaseStoreItemData).ToList();
                case ShopCategoryType.Unit:
                    return UnitCategoryData.category.Find(x => x.id == id).levels
                        .Select(item => item as BaseStoreItemData).ToList();
            }
        ...
        public GameObject GetAssetForLevel(int id, ShopCategoryType categoryType, int level)
        {
            ...
            if (levels != null)
            {
                ...
                throw new Exception("Can't get an asset. The id is: " + id + ", the category is: " + categoryType +
                                    ", the level is: " + level);
            }
        ...
        public bool IsProductionBuilding(int id, ShopCategoryType categoryType, int level)
        {
            var levels = GetLevels(id, categoryType);
            bool isProduction;
            if (levels != null)
            {
                isProduction = ((IProdBuilding)levels.Find(x => ((ILevel)x).GetLevel() == level))
                    .IsProductionBuilding();
                if (isProduction) return isProduction;

                return false;
            }

            throw new Exception("Can't get an info is it production building or not. The id is: " + id +
                                ", the category is: " + categoryType + ", the level is: " + level);
        }

        public BaseStoreItemData GetStructureData(int id, ShopCategoryType categoryType, int level)
        {
            var levels = GetLevels(id, categoryType);
            BaseStoreItemData data;
            if (levels != null)
            {
                data = levels.Find(x => ((ILevel)x).GetLevel() == level);
                if (data != null) return data;
                throw new Exception("Can't get an info about structure. The id is: " + id + ", the category is: " +
                                    categoryType + ", the level is: " + level);
            }
