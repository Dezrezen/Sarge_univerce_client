...
namespace Assets.Scripts.UIControllersAndData.Player{
	...
	public class PlayerInfoController:MonoBehaviour 
	{
		...
		[SerializeField] private TMP_Text _experienceField;
		[SerializeField] private Slider _experienceSlider;
		...
		{
			// TODO implement here
			if (_playerNameField != null) _playerNameField.text = data.PlayerName;
			if (_levelField != null) _levelField.text = data.LevelData.Level.ToString();
			...
			if (_experienceSlider != null)
				_experienceSlider.value =
					data.ExperienceData.CurrentExp / (float)MaxCapControler.GetMaxCap(data.LevelData);
