...
{
    private const int audioSourcesNo = 2; //attached to the camera, generates sounds

    public static SoundFX Instance;

    //[HideInInspector]
    public bool musicOn, soundOn = true, ambientOn = true, weatherOn, dayTime = true, isWinter;
    ...
    private int maxSounds, currentSwitchingAudioSource, nextSwitchingAudioSource = 1;

    private float musicVoume = 0.3f;

    private readonly float maxMusicVoume = 0.3f;

    private readonly float maxSoundVoume = 0.3f;

    private readonly float currentDelta = -0.04f;

    private readonly float nextDelta = 0.02f;

    private readonly float thresholdUp = 0.99f;
    private readonly float thresholdDown = 0.01f;
    ...
    private readonly AudioSource[]
        switchingAudioSource = new AudioSource[audioSourcesNo]; //one fades in, the other out 

    private readonly float[] switchingAudioSourceVolume = new float[audioSourcesNo] { 0, 0 }; //day,night	
    ...
    private void Start()
    {
        ...
        for (var i = 1; i < sources.Length; i++) sources[i].ignoreListenerVolume = true;
    ...
    private void InitializeAmbient()
    {
        ...
        if (ambientOn) InvokeRepeating("StartTheDay", 0.2f, 0.2f);
    }

    private void StartTheDay() //invoke repeating
    {
        SetSwitchingVolume(0, nextDelta);
        if (switchingAudioSourceVolume[0] >= 0.95f) CancelInvoke("StartTheDay");
    ...
    {
        soundSource.PlayOneShot(buildingFinished);
    }

    public void Move()
    {
        soundSource.PlayOneShot(move);
    }
    ...
    public void BuildingBurn()
    {
        ...
        soundSource.clip = fireBurning;
        soundSource.Play();
        soundSource.PlayOneShot(fireBurning);
    }

    public void BuildingExplode()
    {
        soundSource.PlayOneShot(buildingExplosion);
    }

    public void SoldierPlace()
    {
        soundSource.PlayOneShot(bip);
    }
    ...
    {
        maxSounds++;
        if (maxSounds > 5) //allow 6 sounds total - coming from both builder/cop soldiers - otherwise fades
        {
            maxSounds = 0;
            soundsTimerb = false;
        }
    ...
    public void SoldierExplode()
    {
        soundSource.PlayOneShot(soldierExplosion);
    }
